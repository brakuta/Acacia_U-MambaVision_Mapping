...
model configs, train, compare, map. Every config below is in
`tutorial/examples/` and is loaded and built in the repository's test suite.

## 11.1 Acacia crowns from UAV RGB (binary, 2.5 cm, 1024² tiles)

The reference project. `tutorial/examples/acacia_rgb/configs/_base_/dataset.py`
points the generic template at the hand-over dataset (`/data`, `test2` split,
1024² crops, batch 2) and five model configs share it, which allows the
comparison of the paper to be repeated with one command:
...
neighbours; scale jitter 0.5–2.0; CE + 3·Dice; sliding-window testing with a
...
`tutorial/examples/buildings_rgb/`. Differences: 512² crops (buildings are
...
`tutorial/examples/roads_rgb/`. Differences: larger crops for continuity;
...
`tutorial/examples/lulc_multispectral/`. This is the example to study for any
...
Fine-tune rather than retrain: `load_from` the Acacia small checkpoint, new
dataset base with the new species, learning rate 2e-5, 10k iterations
