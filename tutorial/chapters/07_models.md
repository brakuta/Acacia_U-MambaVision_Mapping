...
Guidance from the Acacia benchmark (test mIoU): U-MV 85.3–85.4, SegFormer-B2
85.2, Mask2Former-R50 84.9, U-Net-R50 84.2, PSPNet-R50 81.9, DeepLabV3+-R50
81.1. Transformers and hybrids generalised better to the unseen region. Start a
new project with two baselines from different families (e.g. DeepLabV3+ and
SegFormer-B2) plus U-MV, and let the comparison decide.
...
is absent from SegFormer and U-MV. Set `auxiliary_head=None` to drop it.
...
| `MambaVisionBackbone(pretrained=True)` | team backbone | ImageNet weights from the Hugging Face Hub |
...
`tutorial/templates/project/scripts/run_experiments.sh` does exactly this and
...
1. subclass `mmengine.model.BaseModule`, implement `forward(x)` returning a
   list of feature maps at strides 4/8/16/32;
...
A custom head subclasses `BaseDecodeHead`, implements `forward(inputs)`
returning logits at stride 4 and inherits loss computation, resizing and
prediction (`umv/models/unet_head.py`, 90 lines).
