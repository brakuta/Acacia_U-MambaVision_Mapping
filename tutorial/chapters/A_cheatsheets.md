...
python tools/download_backbones.py            # MambaVision (HF)      python tools/download_zoo_weights.py   # ResNet/MiT/Swin
# data
...
`MambaVisionBackbone`. Heads: `FCNHead`, `PSPHead`, `DepthwiseSeparableASPPHead`,
`UPerHead`, `SegformerHead`, `GenericUNetHead`. Losses: `CrossEntropyLoss`,
`DiceLoss`, `LovaszLoss`, `FocalLoss`, `TverskyLoss`. Datasets:
`BaseSegDataset`, `UAVAcaciaDataset`, `RepeatDataset`, `ConcatDataset`.
