...
| Symptom | Cause | Fix |
|---|---|---|
| `KeyError: 'X is not in the mmseg::model registry'` | custom module not imported, or typo | `custom_imports = dict(imports=['umv'])`; check spelling and scope prefix |
...
| `OSError: cannot connect to huggingface.co` | offline | `tools/download_backbones.py` once; `HF_HUB_OFFLINE=1` |
